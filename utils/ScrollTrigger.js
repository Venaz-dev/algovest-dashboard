  ...
  _onMediaChange = (e) => {
    ...
    if (_lastMediaTick !== tick) {
      ...
      for (; i < _media.length; i += 2) {
        _win.matchMedia(_media[i]).matches
          ? matches.push(i)
          : _revertAll(1, _media[i]); // Firefox doesn't update the "matches" property of the MediaQueryList object correctly - it only does so as it calls its change handler - so we must re-create a media query here to ensure it's accurate.
      }
...
export class ScrollTrigger {
  ...
  init(vars, animation) {
    ...
    let direction = vars.horizontal ? _horizontal : _vertical,
      ...
      toggleActions =
        isToggle && (once ? "play" : vars.toggleActions).split(" "),
    ...
    if (snap) {
      ...
      snapDelayedCall = gsap
        .delayedCall(snap.delay || scrubSmooth / 2 || 0.1, () => {
          if (
            !_lastScrollTime ||
            (_lastScrollTime === scrubScrollTime && !_pointerIsDown)
          ) {
            let totalProgress =
                animation && !isToggle
      ...
      if (clipped !== prevProgress && enabled) {
        ...
        if (pin) {
          ...
          if (!useFixedPosition) {
            ...
            if (pinReparent) {
              ...
              _reparent(
                pin,
                !_refreshing && (isActive || action) ? _body : spacer
              );
            }
        ...
        toggleClass &&
          ...
          _toArray(toggleClass.targets).forEach((el) =>
            el.classList[isActive ? "add" : "remove"](toggleClass.className)
        ...
        if (stateChanged && !_refreshing) {
          ...
          if (toggled || !_limitCallbacks) {
            ...
            once &&
              (clipped === 1 ? self.kill() : (callbacks[toggleState] = 0)); // a callback shouldn't be called again if once is true.
            if (!toggled) {
    ...
    self.enable = () => {
      if (!enabled) {
        ...
        !animation || !animation.add
          ? self.refresh()
          : gsap.delayedCall(0.01, self.refresh) &&
            (change = 0.01) &&
